...
appInteractor2DTemplate::~appInteractor2DTemplate()
{
	...
	// Quad
	m_Renderer->RemoveActor(m_TexturedQuad);
	vtkDEL(m_Quad);
	vtkDEL(m_TexturedQuad);
...
void appInteractor2DTemplate::OnMove(albaEventInteraction *e)
{
	...
	if (m_ParallelView)
	{
		...
		if (m_IsInBound)
		{
			...
			{
				if (m_CurrentPointIndex == 4)
					MoveMeasure(m_CurrentPointIndex, pointCoord);
				else
					EditMeasure(m_CurrentPointIndex, pointCoord);
			}
		}
...
//----------------------------------------------------------------------------
void appInteractor2DTemplate::UpdateTexture()
{
	m_Renderer->RemoveActor(m_TexturedQuad);
	...
	m_Quad->GetPoints()->SetPoint(0, point5);
	m_Quad->GetPoints()->SetPoint(1, point1);
	m_Quad->GetPoints()->SetPoint(2, point2);
	m_Quad->GetPoints()->SetPoint(3, point3);
	m_Quad->GetPoints()->SetPoint(4, point4);
	m_Quad->GetPoints()->SetPoint(5, point1);
	m_Quad->Update();

	m_Renderer->AddActor(m_TexturedQuad);
}

// MEASURE
//----------------------------------------------------------------------------
void appInteractor2DTemplate::AddMeasure(double point1[3], double point2[3], double point3[3], double point4[3])
{
...
//----------------------------------------------------------------------------
void appInteractor2DTemplate::AddLine(double point1[3], double point2[3])
{
	point1[2] = 0;
	point2[2] = 0;
...
//----------------------------------------------------------------------------
void appInteractor2DTemplate::AddTexture()
{
	wxString configDir = appUtils::GetConfigDirectory().c_str();

	// Read the image which will be the texture
	vtkPNGReader *imageReader; 
	vtkNEW(imageReader);
	imageReader->SetFileName(configDir + "\\" + "Mask.png");
	imageReader->Update();
	...
	points->SetPoint(0, point5[0], point5[1], 0.0);
	points->SetPoint(1, point1[0], point1[1], 0.0);
	points->SetPoint(2, point2[0], point2[1], 0.0);
 	points->SetPoint(3, point3[0], point3[1], 0.0);
 	points->SetPoint(4, point4[0], point4[1], 0.0);
	points->SetPoint(5, point1[0], point1[1], 0.0);
  ...
  textureCoordinates->InsertTuple2(5, 0.0, 0.0);
	...
	// PolyData - Quad
	vtkNEW(m_Quad);
	m_Quad->SetPoints(points);
	m_Quad->SetPolys(polygons);
	m_Quad->GetPointData()->SetTCoords(textureCoordinates);
	...
	vtkNEW(quadMapper);
	quadMapper->SetInput(m_Quad);
	
	// Actor
	vtkNEW(m_TexturedQuad);
	m_TexturedQuad->SetPosition(-1, -1, 0);
	m_TexturedQuad->SetMapper(quadMapper);
	m_TexturedQuad->SetTexture(texture);
...
//----------------------------------------------------------------------------
void appInteractor2DTemplate::EditMeasure(int index, double point[3])
{
...
//----------------------------------------------------------------------------
void appInteractor2DTemplate::MoveMeasure(int index, double point[3])
{
	double point1[3]; double point2[3]; double point3[3]; double point4[3];
	double point5[3];
...
//----------------------------------------------------------------------------
void appInteractor2DTemplate::ResetMeasure()
{
	// Points
	//UpdatePointActor(-1, point); // Edit Point Actor
