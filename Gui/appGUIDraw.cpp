...
BEGIN_EVENT_TABLE(appGUIDraw, wxControl)
  EVT_LEFT_DOWN(appGUIDraw::OnMouse)	
  EVT_LEFT_UP(appGUIDraw::OnMouse)	
  EVT_MOTION(appGUIDraw::OnMouse)
  EVT_PAINT(appGUIDraw::OnPaint)
  EVT_COMMAND_RANGE(MINID, MAXID, wxEVT_COMMAND_SLIDER_UPDATED, appGUIDraw::OnRangeModified)
END_EVENT_TABLE()

int     appGUIDraw::m_TriangleWidgetRadius = 5;
wxPoint appGUIDraw::m_TriangleWidgetCenter(0, 5);
wxPoint appGUIDraw::m_TriangleWidgetPoints[3] = { wxPoint(0, 0), wxPoint(5, 10), wxPoint(-5, 10) };

//----------------------------------------------------------------------------
appGUIDraw::appGUIDraw(wxWindow *parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name) : wxControl(parent, id, pos, size, style) 
{ 
  this->Initialize();
...
//----------------------------------------------------------------------------
void appGUIDraw::SetValue(int i, double value) 
{
  assert(i >= 0 && i < 3);
...
//----------------------------------------------------------------------------
void appGUIDraw::SetRange(double *range) 
{
...
//----------------------------------------------------------------------------
void appGUIDraw::OnPaint(wxPaintEvent &event) 
{
  wxPaintDC dc(this);
  PrepareDC(dc);
  dc.BeginDrawing();
...
//----------------------------------------------------------------------------
void appGUIDraw::OnMouse(wxMouseEvent &event)
{
...
//----------------------------------------------------------------------------
void appGUIDraw::OnRangeModified(wxCommandEvent &event)
{
	albaEventMacro(albaEvent(this, this->GetId()));
}

//----------------------------------------------------------------------------
void appGUIDraw::RedrawWidgets(wxDC &dc, bool eraseWidgets)
{
	wxPen darkGreyPen(wxColour(64, 64, 64), 1, wxSOLID);
...
//----------------------------------------------------------------------------
void appGUIDraw::DrawLabel(int x, int y, double val)
{
